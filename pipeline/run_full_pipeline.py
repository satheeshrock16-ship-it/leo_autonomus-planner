...
def run_autonomous_cycle(fetch_live_data: bool = False) -> dict[str, Any]:
    ...
    rel_samples = np.asarray(list(rel_at_tca_by_debris.values()), dtype=float)
    sigma_xyz_km = np.std(rel_samples, axis=0)
    sigma_xyz_km = np.clip(sigma_xyz_km, 5.0, 2000.0)
    covariance_km2 = np.diag(np.square(sigma_xyz_km))
    for norad_id, miss_distance in min_distance_by_debris.items():
        rel_tca = rel_at_tca_by_debris[norad_id]
        pc_each = collision_check.run(rel_tca, covariance_km2)
    ...
    output: dict[str, Any] = {
        ...
        "covariance_diagonal_km2": [float(v) for v in np.diag(covariance_km2)],
